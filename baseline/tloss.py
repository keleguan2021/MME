        ...
        for a_file in tqdm(files):
            ...
            if data_name == 'SEED':
                ...
                for key in movie_ids:
                    ...
                    anchor_indices = np.random.choice(np.arange(trial_data.shape[0]), size=num_to_sample_trial // 2,
                                                      replace=False)
                    trial_selected = []
                    label_selected = []
                    for k in anchor_indices:
                        pos_idx = np.random.randint(np.clip(k - dis, 0, trial_data.shape[0]),
                                                    np.clip(k + dis, 0, trial_data.shape[0]))
                        neg_idx = np.ones(trial_data.shape[0], dtype=np.bool)
                        neg_idx[np.arange(np.clip(k - dis, 0, trial_data.shape[0]),
                                          np.clip(k + dis, 0, trial_data.shape[0]))] = False
...
class TripletLoss(nn.Module):
    ...
    def forward(self, x_a, x_p, x_n):
        return torch.max((torch.abs(x_a - x_p) - torch.abs(x_a - x_n) + self.alpha), 0).mean()
